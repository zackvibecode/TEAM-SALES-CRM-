...
export default function AppLayout({ children, role }: { children: React.ReactNode; role: "admin" | "sales" }) {
  ...
  return (
    <div className="min-h-screen flex app-shell p-0 lg:p-4 lg:gap-4">
      ...
      <aside
        className={cn(
          "fixed top-0 left-0 z-50 h-full w-[260px] flex flex-col sidebar-panel transition-all duration-300 ease-in-out rounded-none lg:rounded-3xl lg:h-[calc(100vh-2rem)]",
          mobileOpen ? "translate-x-0" : "-translate-x-full",
          sidebarOpen
            ? "lg:relative lg:translate-x-0 lg:w-[260px] lg:opacity-100"
      ...
      >
        <div className="px-5 pt-6 pb-4 shrink-0 w-[260px]">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <button
              type="button"
              onClick={closeMobile}
              className="shrink-0 text-slate-400 hover:text-slate-700 p-1.5 rounded-xl hover:bg-white/60 transition lg:hidden"
              title="Close menu"
            ...
            <button
              ...
              className="shrink-0 text-slate-400 hover:text-slate-700 p-1.5 rounded-xl hover:bg-white/60 transition hidden lg:block"
              title="Close sidebar"
        ...
        <div className="p-4 border-t border-white/80 shrink-0 w-[260px]">
          {userEmail && (
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:rounded-3xl overflow-hidden">
        <header className="main-topbar sticky top-0 z-30 shrink-0 rounded-none lg:rounded-t-3xl">
          <div className="h-14 lg:h-16 flex items-center px-4 lg:px-8 gap-2">
            <button
              type="button"
              onClick={() => setMobileOpen(true)}
              className="lg:hidden text-slate-600 p-2 rounded-2xl glass hover:bg-white/80 transition shrink-0"
              title="Open menu"
            ...
            {!sidebarOpen && (
              <button
                type="button"
                onClick={toggleSidebar}
                className="hidden lg:inline-flex text-slate-600 p-2 rounded-2xl glass hover:bg-white/80 transition shrink-0 items-center justify-center"
                title="Open sidebar"
            ...
            <p className="hidden sm:block text-sm font-semibold text-slate-800 min-w-0 truncate">
              Zaqone CRM
